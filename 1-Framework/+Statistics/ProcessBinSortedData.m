function ProcessedData = ProcessBinSortedData(Data, BinIndices, BinCount, AnonymousFunction)
    ...
    % [BinIndices, NumberOfValues]  = SortDataIntoBins(Data(:, 1), Bins, false, false);
    % ProcessedData                 = ProcessBinSortedData(Data, BinIndices, numel(Bins), @(x) mean(x));
    ...
    if size(ProcessedData, 1) < BinCount
        ProcessedData = [ProcessedData; NaN(BinCount - size(ProcessedData, 1), size(Data, 2))];
    end
